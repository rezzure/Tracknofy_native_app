...
const SiteUpdates = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `${backendURL}/api/progress/report?email=${email}`,
        {
          method: "GET",
          headers: {
  ...
  const renderUpdateItem = ({ item }) => (
    <View className="relative pl-6 pb-6 border-l-2 border-blue-200 ml-4">
      ...
      <View className="flex flex-row justify-between items-baseline">
        <Text className="text-base font-medium text-gray-800 flex-shrink mr-2">
          {item.title}
        </Text>
      ...
      <View className="mt-2 text-sm">
        <Text className="text-gray-500">
          Supervisor:{" "}
          <Text className="font-medium text-gray-700">{item.supervisor}</Text>
        </Text>
      </View>
  ...
  // Render each photo in the gallery and wraping React.forwardRef
  const renderPhotoItem = React.forwardRef(({ item, index }, ref) => (
    <TouchableOpacity
      ref={ref}
      className="relative group"
      onPress={() => openLightbox(item)}
...
//       resizeMode="cover"
//     />
//     <View className="absolute bottom-0 left-0 right-0 p-2 bg-black bg-opacity-70 rounded-b-md">
//       <Text className="text-xs text-white truncate">{item.caption}</Text>
//       <Text className="text-2xs text-gray-300">{formatDate(item.date)}</Text>
//     </View>
//   </TouchableOpacity>
